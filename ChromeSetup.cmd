<# :
@echo off&& set "_F0=%~f0"&& set "_PSargs=%*"
fltmc >nul || (powershell "start cmd ('/c \"\"'+$env:_F0+'\" '+$env:_PSargs+'\"') -Verb RunAs"& exit /b)
powershell -ep bypass "&([scriptblock]::Create([IO.File]::ReadAllText('\\?\'+$env:_F0)))" %*& exit /b
: #>
